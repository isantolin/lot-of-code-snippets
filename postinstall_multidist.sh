...
sudo echo -e "python3 -m pip list --outdated --pre --format=freeze | grep -v '^\-e' | cut -d = -f 1  | xargs -n1 pip install -U\ndnf -y update\nLANG=C snap list --all | while read snapname ver rev trk pub notes; do if [[ \$notes = *disabled* ]]; then sudo snap remove '\$snapname' --revision='\$rev'; fi; done" | sudo tee /usr/bin/auto-upgrade-ign.sh
